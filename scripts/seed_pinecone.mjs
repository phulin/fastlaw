...
const embedEndpoint = process.env.PINECONE_EMBED_ENDPOINT ?? DEFAULT_EMBED_ENDPOINT;
...
const embedBatch = async (texts) => {
  for (let attempt = 0; ; attempt += 1) {
    ...
    if (res.status === 429) {
      ...
      const delayMs = retryAfter ? Number(retryAfter) * 1000 : 60000 + attempt * 10000;
      console.log(`Rate limited by Pinecone. Waiting ${Math.round(delayMs / 1000)}s...`);
...
const totalVectorsPlanned = sections.reduce((sum, entry) => sum + entry.chunks.length, 0);
...
const formatSectionRange = (batch) => {
  const start = batch[0].metadata.section_label ?? batch[0].metadata.section_number;
  const end = batch[batch.length - 1].metadata.section_label ?? batch[batch.length - 1].metadata.section_number;
...
for (const { row, chunks } of sections) {
  ...
  while (pending.length >= BATCH_SIZE) {
    ...
    const embeddings = await embedBatch(batch.map((item) => item.metadata.text));
    ...
    console.log(`Upserted ${totalVectors}/${totalVectorsPlanned} vectors (sections ${range}).`);
  }
}

if (pending.length > 0) {
  const embeddings = await embedBatch(pending.map((item) => item.metadata.text));

...
#[tokio::test]
async fn test_adapter_handles_late_title_heading() {
    let mut t = AdapterTestContext::new(UscAdapter, "root");

    // Title heading comes AFTER the first chapter
    let xml = r#"<?xml version="1.0"?>
        <uscDoc xmlns="http://xml.house.gov/schemas/uslm/1.0" identifier="/us/usc/t2">
            <meta><dc:title>Title 2</dc:title></meta>
            <main>
                <chapter identifier="/us/usc/t2/ch1">
                    <num value="1">Chapter 1</num>
                    <heading>Rules of Construction</heading>
                </chapter>
                <title identifier="/us/usc/t2">
                    <num value="2">Title 2</num>
    ...
    let item = QueueItem {
        ...
        level_name: "title".to_string(),
        level_index: 0,
        metadata: serde_json::json!({ "title_num": "2" }),
    };

    t.add_fixture(&item.url, xml);
    t.run_item(item).await;

    // We expect the title node to have the name from the title heading,
    // even though the chapter came first.
    // If the bug exists, this might have "Title 2".
    t.expect_node("root/t2/root")
        .level("title")
        .name("THE CONGRESS");
}

...
const renderAssistant = (payload: Record<string, unknown>) => {
  ...
  answer.innerHTML = marked.parse(String(payload.answer ?? ""));
  ...
  if (Array.isArray(payload.citations) && payload.citations.length) {
    ...
    payload.citations.slice(0, 5).forEach((item: Record<string, unknown>) => {
      ...
      if (link) {
        ...
        card.append(title, snippet, anchor);
      } else {
...
const renderSources = (citations: Array<Record<string, unknown>>) => {
  ...
  citations.slice(0, 6).forEach((item) => {
    ...
    if (link) {
      ...
      anchor.href = link;
      anchor.textContent = "Open section";
      card.append(header, snippet, anchor);
    } else {
...
const renderWorkflow = (steps: Array<{ name: string; detail: string }>, searches: string[]) => {
...
form?.addEventListener("submit", async (event) => {
  ...
  try {
      ...
      const processEvent = (block: string) => {
        ...
        if (eventName === "trace") {
          ...
          if (Array.isArray((data as Record<string, unknown>)?.trace)) {
            renderTrace((data as Record<string, unknown>).trace as Array<Record<string, unknown>>);
          }
        }
      };
    ...
    if (body) {
      const message =
        (error as Error)?.stack ?? (error as Error)?.message ?? "Search request failed.";
      const pre = document.createElement("pre");
